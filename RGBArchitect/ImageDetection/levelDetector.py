...
import operator
"""
imageloaded(jpg) <- string
boardrows <- int
boardcols <-int
imageoutput (png) <- string
scale <-int
boardcols = 21
boardrows = 17
...
def levelDetector(imageloaded, boardrows, boardcols, scale, imageoutput):
    foto = Image.open(imageloaded)
    foto = foto.resize((int(foto.size[0]/scale),int(foto.size[1]/scale)))
    matrix = array(foto)
    anchototal = foto.size[0]
    altototal = foto.size[1]

    #Si, hemos contado cuadraditos
    cuadraditoshorizontales = boardcols
    cuadraditosverticales = boardrows
